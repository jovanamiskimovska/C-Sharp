...
namespace App
{
    class Program
    {
        public static void PrintMovies(List<Movie> allMovies)
        {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                if (option == 1)
                {
                    ...
                    PrintMovies(currentCinema.ListOfMovies);
                ...
                {
                    Console.ForegroundColor = ConsoleColor.Green;
                    Console.WriteLine("You have chosen option 2. Enter your favourite genre: Comedy, Horror, Action, Drama or SciFi");
                    Console.ForegroundColor = ConsoleColor.White;
                    string chosenGenre = Console.ReadLine();

                    if(Enum.TryParse(chosenGenre, out Genre genre))
                    {
                        List<Movie> filteredMovieList = currentCinema.ListOfMovies.Where(x => x.Genre == genre).ToList();
                        Console.ForegroundColor = ConsoleColor.Green;
                        Console.WriteLine($"Choose a movie from the {genre} movies list");

                        PrintMovies(filteredMovieList);
                    ...
                    {
                        throw new Exception("You must enter valid genre");
                    }
                }
                else
                {
